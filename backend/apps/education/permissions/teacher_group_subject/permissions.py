...
class TeacherGroupSubjectPermission(EducationScopedPermission):
    """
    Ограничения доступа к назначениям преподавателей.
    """
    ...
    def has_object_permission(self, request, view, obj) -> bool:
        """
        Проверяет объектный доступ к назначению преподавателя.
        """

        if request.method in SAFE_METHODS:
            return (
                user_can_access_teacher_assignment(user=request.user, obj=obj)
